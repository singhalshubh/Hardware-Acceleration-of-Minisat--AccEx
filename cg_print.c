...
static int search(Sym *sym);
static int searchA(Arc *arc);
...
int index2[] = {3};
int prop_time2[] = {0};
int propself_time2[] = {2.1};
int arc_time2[] = {0.01};
int arc_childtime2[] = {2.1};
int arc2[][2] = {4,5};//first being source, second being destination.
static int 
search(Sym *sym) 
{
  for(int i=0;i<3;i++) {
    if(sym->cg.index == index2[i]) {
      return i;
    }
  }
  return -1;
...
searchA(Arc *arc) 
{
  for(int i=0;i<sizeof(arc2) / sizeof(arc2[0]);i++) {
      if(arc->parent == arc2[i][0] && arc->child == arc2[i][1]) {
        return i;
      }
  }
  return -1;
...
print_cycle (Sym *cyc)
{
  ...
  if(id1 >= 0) {
    char buf[BUFSIZ];
    cyc->cg.prop.self = propself_time2[id1];
    cyc->cg.prop.child = prop_time2[id1];
    sprintf (buf, "[%d]", cyc->cg.index);
    printf (bsd_style_output
      ? "%-6.6s %5.1f %7.2f %11.2f %7lu"
      : "%-6.6s %5.1f %7.2f %7.2f %7lu", buf,
...
cmp_member (Sym *left, Sym *right)
{
  ...
  int id1 = search(left);
  int id2 = search(right);
  if(id1 >=0) {
    left_time = propself_time2[id1] + prop_time2[id1];
  }
   if(id2 >=0) {
    right_time = propself_time2[id2] + prop_time2[id2];
  }
...
print_members (Sym *cyc)
{
  ...
  for (member = cyc->cg.cyc.next; member; member = member->cg.cyc.next)
    {
      int id1 = search(member);
      if(id1 >= 0) {
        member->cg.prop.self = propself_time2[id1];
        member->cg.prop.child = prop_time2[id1];
      }
      printf (bsd_style_output
...
cmp_arc (Arc *left, Arc *right)
{
  ...
  int id1 = searchA(left);
  int id2 = searchA(right);
  if(id1 >= 0) {
    left->time = arc_time2[id1];
    left->child_time = arc_childtime2[id1];
  }
  if(id2 >= 0) {
    right->time = arc_time2[id2];
    right->child_time = arc_childtime2[id2];
  }
  DBG (TIMEDEBUG,
       printf ("[cmp_arc] ");
  ...
  for (arc = child->cg.parents; arc; arc = arc->next_parent)
    {
      int id1 = searchA(arc);
      if(id1 >= 0) {
        arc->time = arc_time2[id1];
        arc->child_time = arc_childtime2[id1];
      }
      parent = arc->parent;
...
print_line (Sym *np)
{
  ...
  int id1 = search(np);
      if(id1 >= 0) {
        np->cg.prop.self = propself_time2[id1];
        np->cg.prop.child = prop_time2[id1];
      }
